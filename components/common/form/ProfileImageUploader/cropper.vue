  ...
  <div class="cropper">
    ...
    >
      ...
      <div
        ...
        :style="loadingStyle"
      >
        <div v-for="i in 12" :key="i" :class="`sk-circle${i} sk-circle`">
          <div
    ...
    <div
      v-if="!isUpload"
      class="drop_area dummy-uploader"
      :class="{ enter: isEnter }"
    >
      <div
        class="select_file"
      ...
      >
        ...
        <p>
          または<button
            ...
            @click.stop="_handleClickOnButton"
          >
            ファイルを選択
          </button>
        </p>
...
export default {
  ...
  mounted() {
    ...
    const supports = this.supportDetection()
    if (!supports.basic) {
  ...
  methods: {
    ...
    zoom(zoomIn = true, acceleration = 1) {
      if (this.passive) return
      const realSpeed = this.zoomSpeed * acceleration
      const speed = this.outputWidth * PCT_PER_ZOOM * realSpeed
      let x = 1
      if (zoomIn) {
        x = 1 + speed
    ...
    applyMetadata(metadata) {
      ...
      const ori = metadata.orientation || this.orientation || 1
    ...
    getMetadata() {
      if (!this.hasImage()) return {}
      const { startX, startY } = this.imgData
    ...
    addClipPlugin(plugin) {
      ...
      if (typeof plugin === 'function' && !this.clipPlugins.includes(plugin)) {
        this.clipPlugins.push(plugin)
      } else {
        throw new Error('Clip plugins should be functions')
      }
    },
    ...
    _setImagePlaceholder() {
      ...
      if (this.$slots.placeholder && this.$slots.placeholder[0]) {
        const vNode = this.$slots.placeholder[0]
        const { tag, elm } = vNode
        if (tag === 'img' && elm) {
          img = elm
        }
    ...
    _setInitial() {
      ...
      if (this.$slots.initial && this.$slots.initial[0]) {
        const vNode = this.$slots.initial[0]
        const { tag, elm } = vNode
        if (tag === 'img' && elm) {
          img = elm
        }
    ...
    _onNewFileIn(file) {
      ...
      if (!this._fileTypeIsValid(file)) {
        ...
        // const type = file.type || file.name.toLowerCase().split('.').pop()
        return false
      }
    ...
    _fileTypeIsValid(file) {
      ...
      const baseMimetype = accept.replace(/\/.*$/, '')
    ...
    _placeImage(applyMetadata) {
      ...
      if (!this.imageSet) {
        ...
        if (/^-?\d+% -?\d+%$/.test(this.initialPosition)) {
          const result = /^(-?\d+)% (-?\d+)%$/.exec(this.initialPosition)
          const x = +result[1] / 100
          const y = +result[2] / 100
    ...
    _handleDragLeave(evt) {
      ...
      if (this.passive || evt.fromElement.closest('.select_file')) return
    ...
    _drawFrame() {
      ...
      this.loading = false
      const ctx = this.ctx
      const { startX, startY, width, height } = this.imgData
...
.croppa-container {
  ...
  background-image: url('~assets/images/placeholder_profile.png');
  &.croppa--has-target {
    background: none;
  }
}
