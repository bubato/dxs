<template>
  <table class="table-condensed">
    <thead>
      <tr>
        <th :colspan="showWeekNumbers ? 6 : 5" class="month">
          {{ year }} / {{ monthName }}
        </th>
        <th class="prev available" tabindex="0" @click="prevMonthClick">
          <span />
        </th>
        <!-- <th
          v-if="showDropdowns"
        ...
        <th class="next available" tabindex="0" @click="nextMonthClick">
          <span />
        </th>
      </tr>
    </thead>
...
export default {
  ...
  computed: {
    ...
    year: {
      ...
      set(value) {
        ...
        const newDate = this.$dateUtil.validateDateRange(
    ...
    months() {
      const monthsData = this.locale.monthNames.map((m, idx) => ({
        label: m,
        value: idx,
      }))

      if (this.maxDate && this.minDate) {
        const y = this.maxDate.getFullYear() - this.minDate.getFullYear()
        if (y < 2) {
          // get months
          const months = []
          if (y < 1) {
  ...
  methods: {
    ...
    changeMonthDate(date, emit = true) {
      const yearMonth = this.$dateUtil.yearMonth(this.currentMonthDate)
      ...
      if (
        emit &&
        yearMonth !== this.$dateUtil.yearMonth(this.currentMonthDate)
      ) {
        this.$emit('change-month', {
